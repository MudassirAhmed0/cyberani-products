...
interface MyLightBoxProps {
  ...
  slideNo: number;
  showNow: boolean;
  setShowNow: (show: boolean) => void;
}

const MyLightBox: React.FC<MyLightBoxProps> = ({
  isAr,
  sources,
  autoPlay,
  slideNo,
  ...
  const [players, setPlayers] = useState<YouTubePlayer[]>([]);
  ...
  const playCurrentVideo = (slideNumber: number) => {
  ...
  const handleSlides = (slideNumber: number) => {
  ...
  const onYouTubeReady: YouTubeProps["onReady"] = (event) => {
    setPlayers((prevPlayers) => [...prevPlayers, event.target]);
  };
  ...
  return (
    ...
    >
      <div className="lightboxNavigation w-full h-full top-0 left-0">
        <div
          onClick={handleClose}
          className="lightboxNavigationTop cursor-pointer z-[5] absolute w-[32px] h-[32px] p-[8px] md:w-[3.5vw] md:h-[3.5vw] bg-[#232323a6] md:p-[0.5vw]"
        >
          <svg
        ...
        {sources.length > 1 && (
          <div className="lightboxSlideNavigation">
            ...
            <button
              ...
              className={`lightboxPrevButton z-[5] cursor-pointer absolute top-[50%] transform translate-y-[-50%] bg-[#232323a6] p-[1vw] md:p-[1vw] w-[32px] h-[32px] p-[8px] md:w-[3.5vw] md:h-[3.5vw] ${
                ...
                >
                  ...
                  {isVideo ? (
                    <video
                      key={index}
                      src={source.source}
                      className="w-full object-cover relative z-[2] max-w-[90vw] md:max-w-[80vw] max-h-[70vh]"
                      playsInline
                      controls
                      loop
                    ...
                    <YouTube
                      ...
                      opts={{
                        ...
                        height: "100%",
                        playerVars: {
                          autoplay: index === currentSlide && autoPlay ? 1 : 0,
                        },
                      }}
                    />
                  ) : (
